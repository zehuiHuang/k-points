...
func main12() {
	...
	//1、首先遍历m*n的每个位置，然后调用dfs方法进行迭代(参数：x，y，入口点；返回空闲区域个数)
	//2、dfs函数中，在进行上下左右遍历之前的各种条件验证：是否访问过？是否偏离了区域？是否有障碍物
	//3、条件都通过后，则判定是否是入口，以及设置为已访问，并返回count, count=1;上下左右遍历时count+=dfs(i,j,*entrance)
	//4、回到主流程，过滤掉入口不唯一的数据，对符合条件的加到ans结果里
	//5、对结果进行空闲数的升序排序
	//6、若结果为空，则返回NULL，若最大入口唯一则正常返回，若有多个一样的最大入口，则只返回数量

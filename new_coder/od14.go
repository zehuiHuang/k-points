...
*/
func main14() {
	//思路：将每个任务的时间消耗按照升序排序(n1,n2,n3...)，由于流水线总是优先选择耗时最短的任务执行，
	//那么第一次选择时一定是将排序后的前m个进行任务同时执行，即m[1]的执行器执行任务1,m[2]执行任务2 ，m[m]执行任务m
	//随着执行，一定是最前面的先执行完，那么一定第m+1这个任务放到m[1]上执行,即m[1]执行m+1的任务，m[2]执行m+2
	//最后得出结论：m[i%m]会执行i的任务
